...
const populateCameras = () => {
	...
	JSON.parse(localStorage.getItem('user')).data.forEach((obj) => {
		let elem = document.createElement('div')
		elem.setAttribute('cameraId', obj.camera.cameraId)
		...
		childName.innerText = `${obj.ward.wardFirstName}'s Class`

		let cameraName = document.createElement('p')

		cameraName.innerText = `${obj.classroom.className}`
...
const beginStream = (cameraId) => {
	...
	// let src = `${server}/parents/feed/${cameraId}`
	let src =
